/*******************************************************************************
 * Vortex 1 - On-Board Flight Computer Code (Patched)
 * Patched: pins, HDC usage, PID timing, servo API consistency, LoRa TX wait
 * Created for: Mishra Rockets
 * Notes: This file is a direct, minimal patch over the original you provided.
 *       - Do not repeat large-block code edits in chat; use this canvas file.
 *       - Bench test everything, verify pin mappings on your Teensy 4.1 hardware.
 ******************************************************************************/
...
//================================================================================
// HARDWARE PINS & CONFIGURATION (PATCHED: avoid pin conflicts)
//================================================================================
const int PIN_SERVO_PITCH = 9;
const int PIN_SERVO_YAW   = 11;   // moved off 10 to avoid SD CS conflict
...
// Using built-in Teensy 4.1 microSD slot (no CS pin required)
// If you ever switch to an external SPI SD module, define its CS pin here.
...
const float SIGMA_ACCEL_NOISE = 0.5; // Process noise.
const float R_ALTITUDE = 10.0;       // Measurement noise.
...
// SERVO CONFIGURATION
const int SERVO_PITCH_CENTER_US = 1500; // preserved if you later want microsecond control
...
//================================================================================
// 3-STATE KALMAN FILTER FUNCTIONS (unchanged logic)
//================================================================================
...
void setup() {
  ...
  // SD init: using the onboard Teensy 4.1 microSD slot
  if (!SD.begin(BUILTIN_SDCARD)) { enterFailureState("SD_INIT_FAIL"); return; }
  dataFile = SD.open("flightlog.csv", FILE_WRITE);
  if (dataFile) {
    dataFile.println("Timestamp,State,RawAlt,KF_Alt,KF_Vel,KF_Accel,Temp,Humidity,Pressure,BattV,BattC,Roll,Pitch,Yaw,LinAccZ,PitchCmd,YawCmd");
  ...
  if (!hdc.begin(0x44, &Wire)) {
    // optional: continue but log
    Serial.println("HDC init failed — temperature/humidity unavailable");
  }

  if (!ina219.begin()) {
    Serial.println("INA219 init failed — battery telemetry may be unavailable");
  }
  ...
  // Servo initialization: use degree-based API consistently
  servoPitch.attach(PIN_SERVO_PITCH);
  servoYaw.attach(PIN_SERVO_YAW);
  servoPitch.write(90);  // center (degrees)
  servoYaw.write(90);
...
void updateStateEstimation() {
    ...
    kalman_predict(dt);
    // Note: linearAccel.z() is in m/s^2 in the BNO055 linear accel vector; verify orientation mapping before flight
...
void handleAscentTvc() {
  if (acceleration < (BURNOUT_ACCEL_G * 9.81)) { // Using filtered acceleration
    if (millis() - stateTimer > BURNOUT_DETECT_MS) {
      ...
      servoPitch.write(90); servoYaw.write(90); stateTimer = millis();
    }
...
void handleCoast() {
    ...
    if (velocity < 0 && rawBaroAltitude < (coastMaxBaroAlt - 1.0)) {
...
void handleAbortedFailure(const char* reason) {
  ...
  servoPitch.write(90); servoYaw.write(90);
...
//================================================================================
// CONTROLLER & UTILITY FUNCTIONS (PATCHED PID to use dt and patched LoRa TX wait)
//================================================================================
void runTvcPidController() {
  static unsigned long last = 0;
  unsigned long now = micros();
  float dt = (last == 0) ? (1.0f / TVC_UPDATE_RATE_HZ) : (now - last) / 1e6f;
  last = now;

  double currentPitch = euler.y(); // confirm mapping experimentally
  double currentYaw   = euler.z();
  double pitchErr = setpointPitch - currentPitch;
  double yawErr   = setpointYaw   - currentYaw;

  // Integral with dt
  pitchIntegral += pitchErr * KI * dt;
  yawIntegral   += yawErr   * KI * dt;

  // Anti-windup
  pitchIntegral = constrain(pitchIntegral, -10, 10);
  yawIntegral   = constrain(yawIntegral,   -10, 10);

  // Derivative (use error derivative / dt)
  float d_pitch = (pitchErr - lastPitchError) / (dt < 1e-6f ? 1e-6f : dt);
  float d_yaw   = (yawErr   - lastYawError)   / (dt < 1e-6f ? 1e-6f : dt);
  lastPitchError = pitchErr; lastYawError = yawErr;

  pidPitchOutput = (KP * pitchErr) + pitchIntegral + (KD * d_pitch);
  pidYawOutput   = (KP * yawErr)   + yawIntegral   + (KD * d_yaw);

  int servoPitchAngle = 90 + constrain((int)round(pidPitchOutput), -SERVO_MAX_ANGLE, SERVO_MAX_ANGLE);
  int servoYawAngle   = 90 + constrain((int)round(pidYawOutput),   -SERVO_MAX_ANGLE, SERVO_MAX_ANGLE);
...
void logData() {
  ...
  if (dataFile) {
    ...
    // Use HDC302x on-demand read API (matches Adafruit example)
    hdc.readTemperatureHumidityOnDemand(temp, humidity, TRIGGERMODE_LP0);

    dataFile.print(millis()); dataFile.print(","); dataFile.print((int)currentState); dataFile.print(",");
...
void transmitTelemetry(const char* message) {
  ...
  rf95.waitPacketSent(); // ensure TX completed before proceeding
