...
GROUND_WIND_SPEED_STD = 0.5 # 50% STD
# Sensor Noise (rad for gyro, m for barometer)
GYRO_NOISE_STD = 0.05 # 5% STD
BAROMETER_NOISE_STD = 0.2 #20% STD
# Actuator Characteristics
ACTUATOR_RESPONSE_TAU_MEAN = 0.075 # Time constant in seconds
ACTUATOR_RESPONSE_TAU_STD = 0.25 # 25% STD
...
if __name__ == "__main__":
    ...
    for i in range(N_RUNS):
        ...
        params = {
            ...
            'ground_wind_speed': np.random.normal(GROUND_WIND_SPEED_MEAN, GROUND_WIND_SPEED_STD),
            'wind_direction': np.random.uniform(0, 360), # degrees
            'gyro_noise_std': GYRO_NOISE_STD,
            'barometer_noise_std': BAROMETER_NOISE_STD,
            'actuator_response_tau': np.random.normal(ACTUATOR_RESPONSE_TAU_MEAN, ACTUATOR_RESPONSE_TAU_STD),
    ...
    if all_results:
        ...
        mean_params = {
            ...
            'yaw_init': YAW_INIT_MEAN,
            'pitch_init': PITCH_INIT_MEAN,
            # Add mean values for new parameters
            'ground_wind_speed': GROUND_WIND_SPEED_MEAN,
            'wind_direction': 0, # Assume a constant direction for the mean run
            'gyro_noise_std': 0.0, # No noise for the mean run
            'barometer_noise_std': 0.0, # No noise for the mean run
            'actuator_response_tau': ACTUATOR_RESPONSE_TAU_MEAN,
            'actuator_precision_noise_std': 0.0, # No noise for the mean run
        }
